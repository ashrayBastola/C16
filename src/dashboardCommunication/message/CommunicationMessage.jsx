...
export default function CommunicationMessage() {
  ...
  const [messages, setMessages] = useState(() => {
    if (!user) return [];
    const saved = localStorage.getItem(`comm_chat_${user.username}`);
  ...
  useEffect(() => {
    if (user && messages.length > 0) {
      localStorage.setItem(`comm_chat_${user.username}`, JSON.stringify(messages));
    }
  }, [messages, user]);

  useEffect(() => {
    if (user) {
      console.log("DEBUG: CommunicationMessage mounted, user:", user);
      const socket = connectSocket(user);

      if (socket) {
        console.log("DEBUG: Socket obtained in CommunicationMessage:", socket.id);
        socket.off("receive_message").on("receive_message", (data) => {
          console.log("DEBUG: Communication received message:", data);
          setMessages((prev) => [...prev, data]);
        });
      } else {
        console.error("DEBUG: Failed to obtain socket in CommunicationMessage");
      }
    }
  }, [user]);
        ...
        <div className="border-r border-white/10 bg-slate-950/30">
          ...
          <div className="max-h-[70vh] overflow-y-auto">
            <div
              ...
              className={`flex cursor-pointer items-center gap-3 border-b border-white/5 px-5 py-4 transition hover:bg-white/5 ${
                selectedUser === "Admin" ? "bg-white/5" : ""
              }`}
            >
            ...
            {messages.map((msg, index) => (
              ...
              >
                ...
                >
                  ...
                  <p>{msg.message}</p>
                  <p className="mt-2 text-[11px] opacity-70 text-right">{msg.timestamp}</p>
                </div>
              </div>
            ))}
